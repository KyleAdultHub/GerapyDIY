          ...
          <el-table-column
            ...
            <template scope="props">
              ...
              <el-button type="success" size="mini" 
                @click="startSomeSpider(project, props.row.name)">
                <i class="fa fa-caret-right"></i>
                run some
              </el-button>
              <el-button class="my-right"                
                type="danger" size="mini" @click="stopAll(jobs[project], props.row.name)">
...
<script type="text/javascript">
  ...
  export default{
    ...
    methods: {
      stopAll(jobs, spider) {
        const vm = this
        jobs.forEach( (element, index, arr) => {
          if (element.spider == spider && element.status != 'finished' && element.status != "pendding") {
            vm.cancelJob(element.id)
            setTimeout( () => {  
            },209)
          }
        })
      },
      startSomeSpider(project, name) {
        const vm = this
        this.$prompt("请输入要启动的进程数","提示", {
