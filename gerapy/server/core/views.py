...
def project_upload(request):
    ...
    :return: json
    """
    upload_path = join(os.path.abspath(join(os.getcwd(), PROJECTS_FOLDER)))
    if request.method == 'POST':
        request_file_data = request.FILES.get('zipfile')
        try:
            if request_file_data:
                zip_file_name = os.path.join(upload_path, request_file_data.name)
                with open(zip_file_name, 'wb') as zip_file:
                    for c in request_file_data.chunks():
                        zip_file.write(c)

                if zipfile.is_zipfile(zip_file_name):
                    fz = zipfile.ZipFile(zip_file_name, 'r')
                    for file in fz.namelist():
                        fz.extract(file, upload_path)
                    fz.close()
                else:
                    return JsonResponse({'result': 'the file to upload must be zip file'})
                os.remove(zip_file_name)
                return HttpResponseRedirect('/#/project')
            return JsonResponse({'result': 'there was no zipfile to upload'})
        except Exception as e:
            return JsonResponse({'result': 'some error was happened, the error information is {}'.format(e)})
